...
| `bump-pixi` | release one or more packages at an externally-chosen version: bump each pixi.toml, one commit, tag `<pkg>@<version>` per package, push — then pair with `recipes-pr` |
